...
describe "When a user visits an item page" do
  context "and there are item reviews" do
  ...
  context "and there are no item reviews" do
    scenario "they do not have the option to sort reviews" do
      item = Fabricate(:item)

      visit(item_path(item))

      expect(page).to_not have_content("Sort reviews")
      expect(page).to_not have_button("Filter reviews")
