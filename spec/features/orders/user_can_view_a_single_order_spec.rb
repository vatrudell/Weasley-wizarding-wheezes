...
describe "when they visit the show page for an order" do
  it "shows item title, price, quantity" do
    ...
    OrderItem.create(order_id: order.id, item_id: item1.id, quantity: 1, price: item1.price)
    OrderItem.create(order_id: order.id, item_id: item2.id, quantity: 1, price: item2.price)
    OrderItem.create(order_id: order.id, item_id: item1.id, quantity: 2, price: item1.price)
